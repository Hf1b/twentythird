...
void run_cr_variance_read(void *dev_data)
{
	...
	/* check spec (temp) */
	if ((cmcs_buf->cr_variance2[0] > 4000) || (cmcs_buf->cr_variance2[1] > 4000) ||
			(cmcs_buf->cr_variance2[2] > 4000)) {
	...
	struct device_attribute *attr, char *buf)
{
	...
	if (data->dt_data->support_open_short_test)
		feature |= INPUT_FEATURE_SUPPORT_OPEN_SHORT_TEST;
	if (data->dt_data->support_mis_calibration_test)
		feature |= INPUT_FEATURE_SUPPORT_MIS_CALIBRATION_TEST;

	input_info(true, &data->client->dev, "%s: %d%s%s%s\n", __func__, feature,
